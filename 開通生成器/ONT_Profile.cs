...
dba-profile m{Bw_Up}_{Bw_Per}% create
    mode sr 
    sla maximum {((int.Parse(Bw_Up) * (1 + double.Parse(Bw_Per) / 100))*1024).ToString("0")}
    apply
 !
";
            }
...
traffic-profile {Sname}_{Mode}_{Bw_Up} create
 ...
 {P4}
  ...
  mapper 1 
   gemport count 1 
   gemport 1 rate-limit upstream {((int.Parse(Bw_Up) * (1 + double.Parse(Bw_Per) / 100)) * 1024).ToString("0")} {((int.Parse(Bw_Up) * (1 + double.Parse(Bw_Per) / 100)) * 1024).ToString("0")}
   gemport 1 rate-limit downstream {((int.Parse(Bw_Down) * (1 + double.Parse(Bw_Per) / 100)) * 1024).ToString("0")} {((int.Parse(Bw_Down) * (1 + double.Parse(Bw_Per) / 100)) * 1024).ToString("0")}
  mapper 2 
   gemport count 1 
  bridge 1 
   ani mapper 1 
